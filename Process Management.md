...
{
 pid_t pid;
 pid = fork();   // step 1: Fork the process
 if(pid < 0)
 {
  perror("fork failed");
  exit(1);
 ...
 {
  printf("Parent exiting, child (daemon) PID = %d\n", pid);
  exit(0);
 }
 if(setsid() < 0)  // step 2: Create a new session and become session leader
 {
  perror("setsid failed");
  exit(1);
 }
 chdir("/");   // step 3: Change working directory to root
 umask(0);   // step 4: Set file permissions mask to 0
 // step 5: Close standard file descriptors
 close(STDIN_FILENO);
 close(STDOUT_FILENO);
 close(STDERR_FILENO);
 while(1)   // step 6: Daemon process logic
 {
  int fd = open("/tmp/daemon_log.txt", O_WRONLY | O_CREAT | O_APPEND, 0600);
  if(fd != -1)
  {
   dprintf(fd, "Daemon process running... PID = %d\n", getpid());
   close(fd);
  }
  sleep(5);   // Run periodically
 }
 return 0;
}
```
